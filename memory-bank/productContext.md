...
- **Problem**: The sp-base web UI project needs programmatic control over the relay — starting/stopping relay, managing destinations, observing status — but sp-rtk-base-relay v2.0 only exposes a CLI/config-file interface
...
- **Impact**: sp-rtk-base-relay becomes a reusable Python library that external applications (sp-base) can embed and control in-process, without sacrificing its standalone CLI capability
