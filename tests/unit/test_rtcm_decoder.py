...
class TestRTCMMessageDecoder:
    ...
    def test_extract_message_id_valid_rtcm_1005(self):
        ...
        data = bytes(
            [
                0xD3,
                0x00,
                0x13,  # Preamble + length
                0x3E,
                0xD0,  # Message ID 1005 (bits: 0011 1110 1101 0000)
                # Payload (17 bytes)
                0x00,
                0x00,
                0x00,
    ...
    def test_extract_message_id_valid_rtcm_1077(self):
        ...
        data = bytes(
            [
                0xD3,
                0x00,
                0x20,  # Preamble + length (32 bytes)
                0x43,
                0x50,  # Message ID 1077 (bits: 0100 0011 0101 0000)
                # Payload (30 bytes)
                *([0x00] * 30),
                # CRC (3 bytes)
                0x00,
    ...
    def test_extract_message_id_valid_rtcm_1087(self):
        ...
        data = bytes(
            [
                0xD3,
                0x00,
                0x25,  # Preamble + length (37 bytes)
                0x43,
                0xF0,  # Message ID 1087 (bits: 0100 0011 1111 0000)
                # Payload (35 bytes)
                *([0x00] * 35),
                # CRC (3 bytes)
                0x00,
    ...
    def test_extract_message_id_valid_rtcm_1230(self):
        ...
        data = bytes(
            [
                0xD3,
                0x00,
                0x1B,  # Preamble + length (27 bytes)
                0x4C,
                0xE0,  # Message ID 1230 (bits: 0100 1100 1110 0000)
                # Payload (25 bytes)
                *([0x00] * 25),
                # CRC (3 bytes)
                0x00,
    ...
    def test_extract_message_id_minimum_valid_message(self):
        ...
        data = bytes(
            [
                0xD3,
                0x00,
                0x02,  # Preamble + length (2 bytes - just message ID)
                0x00,
                0x10,  # Message ID 1 (bits: 0000 0000 0001 0000)
                # CRC (3 bytes)
                0x00,
                0x00,
                0x00,
    ...
    def test_extract_message_id_maximum_id_value(self):
        ...
        data = bytes(
            [
                0xD3,
                0x00,
                0x02,  # Preamble + length
                0xFF,
                0xF0,  # Message ID 4095 (bits: 1111 1111 1111 0000)
                # CRC (3 bytes)
                0x00,
                0x00,
                0x00,
    ...
    def test_extract_message_id_frame_too_short_for_length(self):
        ...
        data = bytes(
            [
                0xD3,
                0x00,
                0x13,  # Preamble + length (19 bytes expected)
                0x3E,
                0xD0,  # Message ID
    ...
    def test_extract_message_length_maximum(self):
        ...
        data = bytes(
            [
                0xD3,
                0x03,
                0xFF,  # Preamble + length (1023 bytes)
                0x00,
                0x00,
            ]
        )
        ...
        for expected_id, byte3, byte4 in common_messages:
            data = bytes(
                [
                    0xD3,
                    0x00,
                    0x10,  # Preamble + length
                    byte3,
                    byte4,  # Message ID bytes
                    *([0x00] * 14),  # Payload
                    0x00,
                    0x00,
                    0x00,  # CRC
    ...
    def test_extract_all_message_ids_single_message(self):
        ...
        data = frame_no_crc + bytes([(crc >> 16) & 0xFF, (crc >> 8) & 0xFF, crc & 0xFF])
    ...
    def test_extract_all_message_ids_two_messages(self):
        ...
        msg1 = msg1_no_crc + bytes(
            [(crc1 >> 16) & 0xFF, (crc1 >> 8) & 0xFF, crc1 & 0xFF]
        )
        ...
        msg2 = msg2_no_crc + bytes(
            [(crc2 >> 16) & 0xFF, (crc2 >> 8) & 0xFF, crc2 & 0xFF]
        )
    ...
    def test_extract_all_message_ids_three_messages(self):
        ...
        msg3 = msg3_no_crc + bytes(
            [(crc3 >> 16) & 0xFF, (crc3 >> 8) & 0xFF, crc3 & 0xFF]
        )
    ...
    def test_extract_all_message_ids_invalid_message_skipped(self):
        ...
        invalid = bytes([0xD4, 0x00, 0x02, 0x43, 0x50, 0x00, 0x00, 0x00])
    ...
    def test_extract_all_message_ids_large_payload(self):
        ...
        msg1_no_crc = bytes(
            [
                0xD3,
                0x01,
                0xF4,  # Preamble + length (500 = 0x01F4)
                0x3E,
                0xD0,  # Message ID 1005
        ...
        msg2_no_crc = bytes(
            [
                0xD3,
                0x00,
                0x64,  # Preamble + length (100 = 0x64)
                0x43,
                0x50,  # Message ID 1077
...
class TestRTCMCRC24Q:
    ...
    def test_extract_message_id_with_crc_validation_valid(self):
        ...
        frame_no_crc = bytes(
            [
                0xD3,
                0x00,
                0x02,  # Header (2 byte payload = message ID only)
                0x3E,
                0xD0,  # Message ID 1005
            ]
        )
    ...
    def test_extract_message_id_without_crc_validation(self):
        ...
        # Frame with wrong CRC
        frame = bytes([0xD3, 0x00, 0x02, 0x3E, 0xD0, 0x00, 0x00, 0x00])
    ...
    def test_extract_message_id_spurious_frame_rejected(self):
        ...
        for frame in spurious_frames:
            ...
            message_id = RTCMMessageDecoder.extract_message_id(frame, validate_crc=True)
            assert message_id is None, (
                f"Spurious frame should be rejected: {frame.hex()}"
            )
    ...
    def test_is_valid_rtcm_frame_length_too_large(self):
        ...
        frame_no_crc = bytes(
            [
                0xD3,
                0x03,
                0xFF,  # Length = 1023 (0x3FF - maximum 10-bit value)
                0x3E,
                0xD0,
    ...
    def test_crc_validation_filters_bluetooth_chunking_artifacts(self):
        ...
        for frame_data in [spurious1, valid_frame, spurious2]:
            msg_id = RTCMMessageDecoder.extract_message_id(
                frame_data, validate_crc=False
            )
